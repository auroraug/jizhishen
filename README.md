...
系统默认不再假设项目已经完工、八阶段资料全部齐备：

- **阶段门禁审查**：付款、变更、计量、合同、验收等动作批准前，检查当前动作所需前置证据并输出“建议放行 / 附条件放行 / 暂缓审批”。
- **新增资料增量审查**：施工日志、送货单、检测报告等批次到达后，只重新处理本批资料并复用上一次截止时点固化的历史事实。
- **全过程全量终审**：仅在竣工结算、年度内控或历史项目审计时检查全过程八阶段完整性。

每次运行固化待审动作、本批资料、历史资料角色、资料截止时间、基线 Run、适用规则和门禁结论；截止时间之后补传的资料不会倒灌历史运行。风险按新运行记录为新增、持续或已解除。
...
访问 `http://工作站IP:8060`；调试控制台为 `/trace`；接口文档为 `/api/docs`。
