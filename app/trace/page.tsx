          ...
          <main className="trace-main">
            {trace ? (
              <>
                ...
                {trace.run.action_snapshot?.action_type&&<section className="trace-action-snapshot"><div><small>待审业务动作</small><b>{trace.run.action_snapshot.action_label}</b><span>{trace.run.action_snapshot.title}</span></div><div><small>资料时间边界</small><b>{trace.run.cutoff_at||'—'}</b><span>基线 Run {trace.run.baseline_run_id||'首个基线'}</span></div><div><small>本批资料</small><b>{trace.run.action_snapshot.current_document_ids?.length||0} 份</b><span>后续资料不会倒灌</span></div><div><small>门禁结论</small><b>{trace.run.decision?.recommendation||'运行中'}</b><span>新增 {trace.run.decision?.risk_transitions?.new||0} · 持续 {trace.run.decision?.risk_transitions?.ongoing||0} · 解除 {trace.run.decision?.risk_transitions?.resolved||0}</span></div></section>}
